...
```bash
# 进行 ssh 连接前先配置好主机名和密码
echo "jElee-PC" > /etc/hostname
passwd
...
# 配置代理地址
proxy_url="192.168.31.180"                      # 代理服务器地址
...
# -o 指定挂载选项
# subvol=@ 指定挂载 @ 子卷
mount -t btrfs -o compress=zstd,subvol=@ /dev/根子卷 /mnt                   # 挂载根子卷
mount --mkdir -t btrfs -o compress=zstd,subvol=@home /dev/根子卷 /mnt/home  # 挂载 home 子卷，由于 /mnt/home 目录可能不存在，使用 --mkdir 选项自动创建
mount --mkdir /dev/启动分区 /mnt/efi                                        # 挂载 ESP(EFI 系统分区)，即启动目录

# 如果需要 swap 分区的话
mount --mkdir -t btrfs -o compress=zstd,subvol=@swap /dev/根子卷 /mnt/swap
...
# vim -> 文本编辑器
# sudo -> 提升权限工具
# intel-ucode -> Intel CPU 微码更新，amd-ucode -> AMD CPU 微码更新
pacstrap -K /mnt networkmanager vim sudo intel-ucode

pacstrap -K /mnt networkmanager vim sudo amd-ucode # 如果是 AMD CPU，则安装 amd-ucode 包
pacstrap -K /mnt base base-devel linux-zen linux-firmware btrfs-progs networkmanager vim sudo intel-ucode # 如果是 Intel CPU，则安装 intel-ucode 包
...
grub-install --target=x86_64-efi --efi-directory=/efi --boot-directory=/efi --bootloader-id=XXXXXXX
...
# loglevel=5 -> 设置内核日志级别为 5，显示更多的信息，便于调试，0-7 级，数字越大信息越多
